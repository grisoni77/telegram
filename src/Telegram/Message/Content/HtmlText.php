...
//        "strong" => "b",
    ...
    public function __construct($text)
    {
        ...
        // converte tag non supportati in supportati
        foreach ($this->tag_conversion as $old => $new) {
            $text = str_replace("<$old>", "<$new>", $text);
            $text = str_replace("</$old>", "</$new>", $text);
        }

        $text = strip_tags($text, "<b><strong><i><em><a><code><pre>");
        $text = self::truncateComplex($text, 4096);
    ...
    public static function truncateComplex($html, $maxLength = 0, $noSplit = true)
    {
        ...
        $ptString = self::truncate($html, $maxLength, $noSplit, $allowHtml = false);
        ...
        if ($ptString == '...')
        {
            ...
            $ptString = self::truncate($stripped, $maxLength, $noSplit, $allowHtml = false);
        }
        ...
        while ($maxLength <= $baseLength)
        {
            ...
            $htmlString = self::truncate($html, $maxLength, $noSplit, $allowHtml = true);
            ...
            $htmlStringToPtString = self::truncate($htmlString, $maxLength, $noSplit, $allowHtml = false);
